...
async function run() {
  ...
  try {
    test.expects('UDP4 server to connect to "udpServer" and receive message.');
  ...
  try {
    test.expects('UDP6 server to connect to "udp6Server" and receive message.');
    ...
    ipc.serveNet(UDP6Addr.address, "udp6").then(async () => {
      ipc.server.on("message", function gotMessage(data, socket) {
        ...
        ipc.server.emit(UDP6Addr, "END");
      });

      //latency issues when running UDP4 via node-cmd
      await delay(transmit_delay);

      ipc.server.emit(UDP6Addr, "message");
    });
